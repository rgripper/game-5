...
type AxisTrackerReducer = (
  tracker: AxisTracker,
  stateToSet: AxisState
) => AxisTracker;
...
export function mapMovementKeysToCommands(
  ...
  return keyEvents$.pipe(
    scan<
      { event: KeyboardEvent; reducer: AxisTrackerReducer },
      MovementTracker
    >(
      (tracker, { event, reducer }) =>
        reduceTrackerWithKey(movementKeys, tracker, event.key, reducer),
...
function reduceTrackerWithKey(
  ...
  switch (key) {
    case movementKeys.back:
      return {
        ...movementTracker,
        vertical: axisStateTrackerReducer(
          movementTracker.vertical,
          AxisState.Positive
        )
      };
    ...
    case movementKeys.right:
      return {
        ...movementTracker,
        horizontal: axisStateTrackerReducer(
          movementTracker.horizontal,
          AxisState.Positive
        )
      };
...
function removeAxisState(
  axisTracker: AxisTracker,
  stateToRemove: AxisState
): AxisTracker {
...
function getDirection(movementTracker: MovementTracker): number {
  // right
  if (
    movementTracker.vertical.active === undefined &&
  ...
  if (
    movementTracker.vertical.active === AxisState.Positive &&
    movementTracker.horizontal.active === undefined
  ) {
    return 90;
  }
