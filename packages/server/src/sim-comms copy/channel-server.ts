...
export function createServerOnChannel({ channel, onStart }: CreateServerOnChannelParams): ChannelServer {
    ...
    return {
        commands: fromEvent<MessageEvent>(channel as any, 'message').pipe(
            map(getCommandFromMessageEvent),
            filter(isDefined),
        ),
        sendCycleOutput: (cycleOutput: SimCycleOutput) =>
            channel.send(JSON.stringify({ type: ServerMessageDataType.CycleOutput, cycleOutput })),
    };
}
...
const _getCommandFromMessageEventOrReplyReady = (params: CreateServerOnChannelParams) => (
    event: MessageEvent,
