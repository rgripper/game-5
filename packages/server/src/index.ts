...
import { ApolloServer, gql, IResolvers, AuthenticationError, PubSub } from 'apollo-server';
...
const auth = <TObj, TArgs, TContext extends { userId: string }, TResult>(func: MutationResolver<TContext, TResult>) => {
...
const apolloServer = new ApolloServer({
    ...
    context: ({ req, connection }) => {
        const authToken = req?.headers.authorization ?? connection?.context?.authToken;
    ...
    resolvers: {
        Query: {
            players: (object: unknown, { name }: { name: string }, context: CustomContext) => {
                return context.roomService.getPlayers();
            },
        },
        Mutation: {
            login: (object: unknown, { name }: { name: string }, context: CustomContext) => {
